...
- 1단계 완료: 스무딩(주파수 분리 바이래터럴) + 브라이트닝 + 하이라이트 롤오프
- 2단계 완료: 랜드마크 기반 MLS 계열 워핑 — 눈/갸름(측면 윤곽 전체)/코
- 톤 보정, 눈밑(다크서클) 별도 채널, 스킨톤 게이팅
- 다중 얼굴: numFaces 3, 얼굴별 위치 추적 + One Euro Filter 스무딩
- 블렌드쉐이프: eyeBlink로 눈 감을 때 눈 확대 자동 감쇠
- 감지 주기: 카메라 240ms / 편집 900ms, 보정값 0이면 감지 비용 없음
- UI: 보정 전용 패널(보정리셋/자동/피부/톤/눈밑/눈/갸름/코), 원본보기 press-and-hold
